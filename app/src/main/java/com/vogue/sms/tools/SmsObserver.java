        ...
        try {
            ...
            if (c != null) {
                if (c.moveToFirst()) {
                    ...
                    int type = c.getInt(c.getColumnIndex("type"));// 1,表示发送短信，2是表示接收短信
                    long date = c.getLong(c.getColumnIndex("date"));//时间
                    //将日期格式化，按这个模式输出日期
                    ...
                    Log.e("TTTTT",address+"  "+body+"  "+type+"  "+date);
                    if (mHandler != null) {
                        mHandler.obtainMessage(MSG_RECEIVED_CODE, new String[]{address,date+"", body}).sendToTarget();
                    }
